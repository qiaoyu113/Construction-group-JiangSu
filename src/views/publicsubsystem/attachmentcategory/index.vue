...
<div>
  ...
  <exist-category-select-form v-if="existCategorySelectVisible" ref="existCategorySelectForm" @change="doRefresh"></exist-category-select-form>
</div>
</template>
<script>
  ...
  methods: {
    ...
    doBatchDelete() {
      ...
      self.$confirm('确认要删除共' + ids.length + '项吗?', '提示', {
        ...
        tapp.services.base_Attachment.batchDeleteClassificationOfCategories({
          'categoryIds':ids,
          'categoryClassificationId':self.selectedCategoryItem.id,
