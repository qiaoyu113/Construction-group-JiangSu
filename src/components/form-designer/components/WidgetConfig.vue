...
<div v-if="show">
  <el-form label-position="top" size="small">
    <el-form-item label="标题" v-if="!data.formType || data.formType == 'input'">
    ...
    <el-form-item label="附加文本" v-if="Object.keys(data.options).indexOf('appendText')>=0">
      <el-input v-model="data.options.appendText"></el-input>
    </el-form-item>
    <el-form-item label="单位" v-if="Object.keys(data.options).indexOf('unitValue')>=0">
      <el-input v-model="data.options.unitValue" type="number"></el-input>
    </el-form-item>
    ...
    <el-form-item label="选项" v-if="data.type=='tree'">
      <div>
        <t-dic-category-cascader v-model="data.options.dataDicCategoryValue" />
        <template v-if="data.type=='radio' || (data.type=='select'&&!data.options.multiple)">
            <t-dic-radio-select v-if="data.options.dataDicCategory && data.options.dataDicCategoryValue"  :dicType="data.options.dataDicCategoryValue" v-model="data.options.defaultValue"></t-dic-radio-select>
            <el-button @click="data.options.defaultValue = null">清理默认值</el-button>
          </template>
        <template v-if="data.type=='checkbox' || (data.type=='select' && data.options.multiple)">
            <t-dic-checkbox-select v-if="data.options.dataDicCategory && data.options.dataDicCategoryValue"  :dicType="data.options.dataDicCategoryValue" v-model="data.options.defaultValue"></t-dic-checkbox-select>
          </template>
      </div>
    </el-form-item>

    <el-form-item label="选项" v-if="Object.keys(data.options).indexOf('options')>=0">
      <el-radio-group v-model="data.options.dataDicCategory" size="mini" style="margin-bottom:10px;">
        <el-radio-button :label="false">静态数据</el-radio-button>
        <el-radio-button :label="true">数据字典</el-radio-button>
      </el-radio-group>
      <template v-if="data.options.dataDicCategory">
          <div>
          <t-dic-category-cascader  v-model="data.options.dataDicCategoryValue"/>
          <template v-if="data.type=='radio' || (data.type=='select'&&!data.options.multiple)">
            <t-dic-radio-select v-if="data.options.dataDicCategory && data.options.dataDicCategoryValue"  :dicType="data.options.dataDicCategoryValue" v-model="data.options.defaultValue"></t-dic-radio-select>
...
<template v-if="data.type=='radio' || (data.type=='select'&&!data.options.multiple)">
  ...
  <div v-if="data.options.dataDicCategory && data.options.dataDicCategoryValue"  >

  </div>
  <el-radio-group v-model="data.options.defaultValue" v-else>
...
<el-form-item label="默认值" v-if="Object.keys(data.options).indexOf('defaultValue')>=0 && (data.type == 'textarea' || data.type == 'input' || data.type=='currency' || data.type=='int' || data.type=='percent' || data.type=='number' || data.type=='switch')">
  ...
  <t-int-input v-if="data.type == 'int'" v-model="data.options.defaultValue" ></t-int-input>
  <t-currency-input v-if="data.type == 'currency'" v-model="data.options.defaultValue" ></t-currency-input>
  <t-percent-input v-if="data.type == 'percent'" v-model="data.options.defaultValue" ></t-percent-input>
  <t-number-input v-if="data.type == 'number'" v-model="data.options.defaultValue" ></t-number-input>
...
<el-form-item label="描述">
  <el-input v-model="data.options.description"></el-input>
</el-form-item>
...
<el-form-item label="列配置项" v-if="Object.keys(data).indexOf('columns')>=0">
...
<template v-if="!data.formType || data.formType == 'input' ">
        <el-form-item label="数据绑定Key" v-if="data.model != undefined">
          <el-input v-model="data.model"></el-input>
        </el-form-item>
        <el-form-item label="操作属性" v-if="Object.keys(data.options).indexOf('readonly')>=0
        ...
        <el-form-item label="必填" v-if="Object.keys(data.options).indexOf('required')>=0" >
          <div>
            <el-checkbox v-model="data.options.required" >必填</el-checkbox>
          </div>
        </el-form-item>
          <el-form-item label="最大长度" v-if="Object.keys(data.options).indexOf('maxLength')>=0">
            <el-input-number v-model="data.options.maxLength" :min="0" :max="4000" :step="1"></el-input-number>
          </el-form-item>
